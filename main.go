package main
 ...
 func dbConn() (db *sql.DB) {
   ...
   db, err := sql.Open("postgres", "postgres://user:password@hostname/dbname?sslmode=disable")
 ...
 db := dbConn()
 nId := r.FormValue("name")
 selDB, err := db.Query("SELECT * FROM Employee WHERE name =$1;", nId)
 if err != nil {
 panic(err.Error())
 }
 ...
 tmpl.ExecuteTemplate(w, "Search", emp)
 defer db.Close()
